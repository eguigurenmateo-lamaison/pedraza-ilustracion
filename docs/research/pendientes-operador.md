...
| 1.3 | ✅ **RESUELTO (Felipe, cliente, jul-2026):** Confirmación de la política de envío vigente y el umbral de envío gratis real — **envío gratis sobre $50.000 CLP**; el $30.000 CLP que aparecía en una ficha de producto individual era caché desactualizada del buscador. Actualizado en `tienda.md` secciones 5 y 8 | Se detectó una inconsistencia entre **$50.000 CLP** (mencionado en el home) y **$30.000 CLP** (mencionado en una ficha de producto individual) — `tienda.md` sección 5 | Texto (captura de la página de envíos o respuesta directa) |
...
| 1.6 | ✅ **RESUELTO (operador, jul-2026):** Confirmación de quién es "Cote" (cote@pedrazailustracion.com) y si es el canal de atención al cliente actual — **Cote es María José Pedraza, y es quien responde DMs y correos hoy.** Actualizado en `tienda.md` sección 4 | Saber quién responde DMs/mails hoy, relevante para diseñar flujos de automatización de DM (ver `mercado-tendencias.md` sección C.3.4) | Texto |
...
| 2.3 | ✅ **RESUELTO (operador, jul-2026):** Conteo actual de seguidores de @pedraza_ilustracion — **62.500 seguidores.** Actualizado en `instagram.md` secciones 1 y 7. Sigue pendiente la evolución histórica, si está disponible (Meta Business Suite muestra crecimiento neto por período) | No se encontró en ningún resultado de búsqueda indexado (`instagram.md` sección 1); es el número base para cualquier objetivo de crecimiento del plan | Captura de pantalla |
...
| # | Qué se necesita | Para qué se usa en el plan | Formato sugerido |
|---|---|---|---|
| 3.1 | **Parcialmente resuelto (Felipe, cliente, jul-2026):** la marca **"Pedraza Ilustración" ya está registrada en Chile (INAPI)** 🟢. **[PRIORIDAD] sigue pendiente el registro en EEUU (USPTO)** — Felipe: "Tenemos que registrarla también en EEUU"; es decisión ya tomada por el cliente, falta solo iniciar/completar el trámite | Es la dependencia crítica y bloqueante de calendario para todo el anexo Amazon: sin marca registrada (o en trámite) en EEUU no hay Brand Registry, y sin Brand Registry no hay A+ Content, Amazon Store, Amazon Attribution ni Brand Referral Bonus (`mercado-tendencias.md` secciones D.2.6 y D.4). El trámite puede tardar meses y debería iniciarse ya si se quiere "abrir pronto" en Amazon | Texto / documentación del trámite USPTO cuando se inicie |
...
- A medida que se resuelva cada ítem, el dato correspondiente debe actualizarse directamente en `tienda.md`, `instagram.md` o `mercado-tendencias.md`, reemplazando el marcador 🟡 por el dato confirmado (no dejar ambos), y la fila correspondiente en esta lista se marca ✅ RESUELTO (sin borrarla, como registro). Los ítems **1.6** (quién es Cote) y **2.3** (seguidores de Instagram) ya se resolvieron así con el operador en julio de 2026 — quedan como ejemplo del formato a seguir para el resto de la lista.
