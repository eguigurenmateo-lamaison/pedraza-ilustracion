...
|---|---|---|
| WebFetch a `https://pedrazailustracion.com/products.json?limit=250` | ❌ Falló | HTTP 403 Forbidden devuelto por el propio sitio |
| WebFetch a `https://pedrazailustracion.com/collections/all/products.json?limit=250` | ❌ Falló | HTTP 403 Forbidden |
| WebFetch a `https://pedrazailustracion.com/collections.json` | ❌ Falló | HTTP 403 Forbidden |
| WebFetch a `https://pedrazailustracion.com/sitemap.xml` | ❌ Falló | HTTP 403 Forbidden |
| WebFetch a `https://pedrazailustracion.com/robots.txt` | ❌ Falló | HTTP 403 Forbidden |
| WebFetch al home (`/`) y a páginas de colección/producto individuales | ❌ Falló | HTTP 403 Forbidden en todos los casos probados (home, `/collections/laminas`, `/products/lamina-mariposas-de-chile`) |
| `curl` directo desde bash (con User-Agent de navegador) | ❌ Falló, pero por otra causa | El proxy de egress de la sesión rechazó la conexión a `pedrazailustracion.com:443` con 403 a nivel de CONNECT ("policy denial or upstream failure") — es decir, ni siquiera llegó a golpear el sitio. Confirmado con `curl http://127.0.0.1:34557/__agentproxy/status`, que registra el host bloqueado en `recentRelayFailures`. |
| Proxy de lectura de terceros `r.jina.ai` (bypass común de 403 anti-bot) vía WebFetch | ❌ Falló | También devolvió 403 |
| WebFetch a fichas de reseller (mercadolibre.cl, bazared.cl) | ❌ Falló | HTTP 403 Forbidden — el bloqueo de fetch directo no es exclusivo del dominio del cliente, varias plataformas de e-commerce chilenas bloquean bots de forma similar |
...
**Conclusión operativa para futuras fases de research sobre este cliente:** el sitio (Shopify) tiene un bloqueo anti-bot que afecta tanto a fetch directo como a endpoints JSON públicos y a proxies de lectura de terceros — no es viable levantar el catálogo completo ni verificar precios en tiempo real sin acceso directo (ej. credenciales de Shopify admin, o un export/CSV del operador). Todo intento futuro debería apoyarse en WebSearch dirigido por categoría/producto, complementado con lo que el operador pueda exportar directamente.
